...
def replace_env(matchobj):
    ...
    assert  key in os.environ,"config file is using a key not defined as environment variable: "+key
...
if len(sys.argv) == 2: 
    ...
    outputfile = None
    print(f"Mutate embeddings in cluster.")
...
if confirm == "y":
    ...
    with open("./embeddings.json") as f:
        ...
        for embedding_def in definitions:                
            total = buildEmbeddings(
                    embedding_def,
                    only_missing,
                    out)
            print(f"{total} embeddings for {embedding_def['entityType']}.{embedding_def['attribute']}")
        if out != None:
            out.close()
