...
class DgraphClientStub(object):
    ...
    def __init__(self, addr='localhost:9080', credentials=None, options=None):
        if credentials is None:
            self.channel = aio.insecure_channel(addr, options)
        else:
            self.channel = aio.secure_channel(addr, credentials, options)

        self.stub = api_grpc.DgraphStub(self.channel)

    async def login(self, login_req, timeout=None, metadata=None, credentials=None):
        return await self.stub.Login(login_req, timeout=timeout, metadata=metadata,
                               credentials=credentials)

    async def alter(self, operation, timeout=None, metadata=None, credentials=None):
        """Runs alter operation."""
        return await self.stub.Alter(operation, timeout=timeout, metadata=metadata,
                               credentials=credentials)
    ...
    async def query(self, req, timeout=None, metadata=None, credentials=None):
        ...
        return await self.stub.Query(req, timeout=timeout, metadata=metadata,
                               credentials=credentials)
    ...
    async def commit_or_abort(self, ctx, timeout=None, metadata=None,
                        credentials=None):
        """Runs commit or abort operation."""
        return await self.stub.CommitOrAbort(ctx, timeout=timeout, metadata=metadata,
                                       credentials=credentials)

    async def check_version(self, check, timeout=None, metadata=None,
                      credentials=None):
        """Returns the version of the Dgraph instance."""
        return await self.stub.CheckVersion(check, timeout=timeout,
                                      metadata=metadata,
                                      credentials=credentials)
    ...
    def from_slash_endpoint(cloud_endpoint, api_key):
        return DgraphClientStub.from_cloud(cloud_endpoint, api_key)
