...
class DgraphClient(object):
    ...
    async def check_version(self, timeout=None, metadata=None, credentials=None):
        ...
        try:
            response = await self.any_client().check_version(check_req, timeout=timeout,
    ...
    async def login(self, userid, password, timeout=None, metadata=None,
              credentials=None):
        ...
        response = await self.any_client().login(login_req, timeout=timeout,
                                           metadata=metadata,
                                           credentials=credentials)
        self._jwt = api.Jwt()
    ...
    async def login_into_namespace(self, userid, password, namespace, timeout=None, metadata=None,
              credentials=None):
        login_req = api.LoginRequest()
        login_req.userid = userid
        login_req.password = password
    ...
    async def alter(self, operation, timeout=None, metadata=None, credentials=None):
        ...
        try:
            return await self.any_client().alter(operation, timeout=timeout,
                                           metadata=new_metadata,
                                           credentials=credentials)
        except Exception as error:
            if util.is_jwt_expired(error):
