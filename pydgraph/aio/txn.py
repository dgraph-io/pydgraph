...
class Txn(object):
    ...
    async def query(self, query, variables=None, timeout=None, metadata=None, credentials=None, resp_format="JSON"):
        """Executes a query operation."""
        req = self.create_request(query=query, variables=variables, resp_format=resp_format)
        return await self.do_request(req, timeout=timeout, metadata=metadata, credentials=credentials)

    async def mutate(self, mutation=None, set_obj=None, del_obj=None,
    ...
    async def do_request(self, request, timeout=None, metadata=None, credentials=None):
        ...
        query_error = None
        try:
            response = await self._dc.query(request, timeout=timeout,
        ...
        if query_error is not None:
            try:
                await self.discard(timeout=timeout, metadata=metadata,
                             credentials=credentials)
            except:
    ...
    async def commit(self, timeout=None, metadata=None, credentials=None):
        ...
        new_metadata = self._dg.add_login_metadata(metadata)
        try:
            await self._dc.commit_or_abort(self._ctx, timeout=timeout,
                                     metadata=new_metadata,
                                     credentials=credentials)
        except Exception as error:
            if util.is_jwt_expired(error):
